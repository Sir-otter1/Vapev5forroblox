...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function() return game:HttpGet('https://raw.githubusercontent.com/Sir-otter1/Vapev5forroblox/'..((isfile('newvape/profiles/commit.txt') and readfile('newvape/profiles/commit.txt') or ''):match('^%x+$') and readfile('newvape/profiles/commit.txt') or 'main')..'/'..select(1, path:gsub('newvape/', '')), true) end)
